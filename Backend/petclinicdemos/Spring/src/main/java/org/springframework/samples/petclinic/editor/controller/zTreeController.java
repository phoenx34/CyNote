...
import org.springframework.samples.petclinic.editor.utils.SimpleTreeNode;
...
//            lstTree.add(new SimpleTreeNode(id,1,group.getGroupName().split("@")[0]));
            if (group.getMember() == null) {
                lstTree.add(new SimpleTreeNode(id,1,group.getGroup()));
                pid = id;
                id++;
                //list pad
...
//                https://www.cnblogs.com/mingforyou/archive/2013/09/03/3299569.html
        ...
        for(int i=0;i<gsm.size();i++) {
            group = gsm.get(i);
//            System.out.println(group.ToString());
            lstTree.add(new SimpleTreeNode(id,2,group.getGroup()));
            pid = id;
            id++;
            //list pad
            result = client.createGroupIfNotExistsFor(group.getGroup());
